...
- **输入栏省位**：移动端权限选择器只留盾形图标（标签与箭头收起，紧贴 `+` 按钮），模型名限宽（手机 112px / 宽屏 200px），超长时**单向无缝循环跑马灯**（双副本 + GPU transform，尾部滚出头部无缝衔接；`prefers-reduced-motion` 下回退省略号），上下文圆环与权限/`+`/发送统一 36px 圆形、固定座位不挤压，上下文明细面板限宽不溢出——任何宽度都不再挤压、变形或重叠
