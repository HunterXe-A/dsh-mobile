...
- **输入栏省位**：移动端权限选择器只留盾形图标（标签与箭头收起，紧贴 `+` 按钮），模型名限宽（手机 112px / 宽屏 200px），超长时**自动跑马灯播放**显示全名（`prefers-reduced-motion` 下回退省略号），右侧簇允许收缩让模型名吸收亏空——`+`/权限/上下文环/发送全部固定尺寸，任何宽度都不再挤压、变形或重叠
